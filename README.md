...
[![Binder badge](https://static.mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/jupyter/docker-stacks/main?urlpath=lab/tree/README.ipynb "Launch a ghcr.io/data-beam/base-notebook container on mybinder.org")

Jupyter Docker Stacks are a set of ready-to-run [Docker images](https://ghcr.io/organization/jupyter) containing Jupyter applications and interactive computing tools.
...
You can [try a relatively recent build of the ghcr.io/data-beam/base-notebook image on mybinder.org](https://mybinder.org/v2/gh/jupyter/docker-stacks/main?urlpath=lab/tree/README.ipynb).
...
```{note}
Since `2023-10-20` our images are only pushed to `ghcr.io` registry.
Older images are available on Docker Hub, but they will no longer be updated.
```

### Example 1

This command pulls the `jupyter/scipy-notebook` image tagged `2024-02-24` from ghcr.io if it is not already present on the local host.
...
```bash
docker run -p 10000:8888 ghcr.io/data-beam/scipy-notebook:2024-02-24
```
...
This command pulls the `jupyter/datascience-notebook` image tagged `2024-02-24` from ghcr.io if it is not already present on the local host.
...
```bash
docker run -it --rm -p 10000:8888 -v "${PWD}":/home/jovyan/work ghcr.io/data-beam/datascience-notebook:2024-02-24
```
...
- [Images on ghcr.io](https://ghcr.io/organization/jupyter)
